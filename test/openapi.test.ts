...
describe("openapi", () => {
  test("adds gradle-semantic-release-plugin, semantic-release/npm, semantic-release/git, and semantic-release-openapi plugins", async () => {
    ...
    const gradlePlugin = openapi.plugins[5];
    const npmPlugin = openapi.plugins[6];
    const openApiPlugin = openapi.plugins[7];
    const semanticReleasePlugin = openapi.plugins[8];
